...
define dns::view (
  ...
  $rfc1912_zones_cfg = $dns::rfc1912_zones_cfg

  concat { "${dns::params::cfg_dir}/view-${name}.conf":
    ensure         => $ensure,
    owner          => $dns::owner,
    group          => $dns::group,
    mode           => '0644',
  ...
  if $ensure == 'present' {
    concat::fragment {"view-${name}.header":
      target  => "${dns::params::cfg_dir}/view-${name}.conf",
      order   =>  '00',
    ...
    concat::fragment {"named.conf.local.view.${name}.include":
      target  => "${dns::params::cfg_dir}/named.conf.local",
      order   => $order,
      content => "include \"${dns::params::cfg_dir}/view-${name}.conf\";\n",
      require => Concat["${dns::params::cfg_dir}/view-${name}.conf"],
    }
